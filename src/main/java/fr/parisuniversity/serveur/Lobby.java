...
public class Lobby implements Runnable {
    ...
    public String getListFromGame(byte pgameNum) {
        ...
        try {
            Game game = null;
            synchronized (AppServeur.games) {
                game = AppServeur.getGame(AppServeur.games, pgameNum);
    ...
    /*
     * Envoyer au serveur La Listes des Games qui ne sont pas encore commencer
     * existantes
     */
    ...
    public String sendListGames() {
        ...
        System.out.println(nombreOfInstartedGames);
        ...
        for (int i = 0; i < (byte) (AppServeur.games.size()); i++) {
            if (!AppServeur.games.get(i).getIsStarted()) {
                ...
                messagesended += msg + String.valueOf(AppServeur.games.get(i).getnumGame())
                        + " " + String.valueOf(AppServeur.games.get(i).getGamePlayers().size()) + " ***";
    ...
    private boolean saftyRead(int size) {
        ...
        try {
            /*
             * Lire sur le flux de la socket client si le message contient *** du protocole
             * (lecture terminer)
